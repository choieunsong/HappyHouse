<template>
  <div style="margin: 40px 0 40px;">
    ...
    <div class="shadow p-3 mb-5 bg-white rounded">
      ...
      <div class="text-center" v-if="getUserState == 1">
...
export default {
  ...
  methods:{
    updateBoard(){
      this.$router.push('/happyhouse/update');
      setTimeout(()=>{
        bus.$emit('updateBoard',this.board)
      },100)
    },
    removeBoard(){
      this.$router.push('/happyhouse/remove')
      setTimeout(()=>{
    ...
    searchBoard(no){
      ...
      axios
      .get(`http://localhost:9090/happyhouse/notice/board/${no}`)
      .then(({ data }) => {
  ...
  insertComment(){
        ...
        axios
        .post(`http://localhost:9090/happyhouse/notice/board/reply`,{
            no: 0,
            bno: this.board.no,
